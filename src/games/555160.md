...
opinions:
    ...
    - steamVR: 5
      ...
      text: |
          Previously played successfully with no issues.
          Now, tried to play with flatpak WiVRn and the game had sound but just a black screen. 
          I then tried the AUR version of WiVRn, no dice. Same issue.
          I tried with `-dx11` and `-dx12` as launch options, also no dice.
          I then tried ALVR, which got me to the Vankrupt logo and disclaimer, but then it crashed my entire desktop session when getting into the main menu. I did not see a single frame of the main menu inside the headset, only on the PC screen. 
          Then I tried SteamVR with Steam Link VR, and it actually almost worked too! It got through the logos, but crashed with the following error in the main menu (and once again didn't see a single frame of the main menu in the headset, only on the monitor)
          <img width="728" height="322" alt="Image" src="https://github.com/user-attachments/assets/aa6b2eb6-6ab7-4d22-8f94-8ccbf98a8766" />
          Sometimes starting it with WiVRn results in the game not starting in VR, but then boots successfully into the main menu, although not in VR so it is not playable. 
      date: 2025-10-03
      user: Rosentti
---
