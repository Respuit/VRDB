...
opinions:
    ...
    - steamVR: 0
      ...
      device: PICO 4
      text: |
          `OXR_VIEWPORT_SCALE_PERCENTAGE=50 %command%`
          Works on XRizer (using `MrZero-FBT`/`experimental2`), sometimes crashes after the splash screen but this seems to be resolved with the changes in the `hitman` branch.
          Proton: GE-Proton7-55
          Controllers display as Vive Wands and performance seems sub-par, but otherwise it's perfectly playable.
      date: 2025-10-21
      user: cubee-cb
---
